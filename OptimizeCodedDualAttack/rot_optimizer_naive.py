...
def optimize_from(alpha, q, n, red_cost_model, target_proba_false_candidate_global, target_proba_senu, L_param, option_dlsc, lock_nfft_kfft):
	if(not lock_nfft_kfft):
		around_m = 1
		around_nenu = 6 # increase search range nenu as our guessing strategy is improved
		around_nfft = 1
		around_kfft = 1
	else:
		around_m = 3
...
def optimize(res_starting_point, nb_iteration_optimiser, nb_core, option_dlsc):
    ...
    with mp.Pool(processes=nb_core, maxtasksperchild=1) as pool:
        # stream results instead of building LL
        for scheme, nn, L_best, Comp_best in pool.imap_unordered(func_, range(nbS), chunksize=1):
            ...
            n = cst.n
            nlat = n - nenu - nfft
            alpha = 3 if n == 512 else 2
            option_dlsc_ = option_dlsc[scheme][nn]
            d_comp = complexity(alpha, q, m, nenu, nlat, nfft, kfft,
                                red_cost_model,
                                target_proba_false_candidate_global,
                                target_proba_senu,
                                option_dlsc_,
...
def generate_data_polar_code(filename, output, nb_core=4):
    ...
    with mp.Pool(processes=nb_core, maxtasksperchild=1) as pool:
        for scheme, nn, d in pool.imap_unordered(func_, range(nbS), chunksize=1):
            resB[scheme][nn] = d

    resF = {0: res, 1: resB}
    with open(output, 'wb') as f:
        pickle.dump(resF, f, protocol=pickle.HIGHEST_PROTOCOL)
...
def optimize_from_starting_parameter_with_experimental_polar_code(filename_starting_point,filename_output):
	...
	res = optimize(res, nb_iteration_optimiser, nb_core = 4, option_dlsc = option_dlsc)
...
if __name__ == "__main__": 
	...
	filename_output1 = 'rotated_optimized_withoutExperimentalPolar.pkl'
	filename_output2 = 'rotated_optimized_withoutExperimentalPolar_StatisticsPolar.pkl'
	filename_output3 = 'rotated_optimized_withExperimentalPolar.pkl'
